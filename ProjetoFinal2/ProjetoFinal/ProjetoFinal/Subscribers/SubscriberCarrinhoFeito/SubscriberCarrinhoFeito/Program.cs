...
class SubscriberCarrinhoFeito
{
    static async Task Main(string[] args)
    {
        ...
        consumer.Received += async (model, ea) =>
        {
            ...
            try
            {
                var pedidoJson = JsonDocument.Parse(mensagem);
                var itens = pedidoJson.RootElement.GetProperty("Itens");

                using var httpClient = new HttpClient();

                var content = new StringContent(mensagem, Encoding.UTF8, "application/json");

                var response = await httpClient.PutAsync("http://api_produto:5000/api/Produto/estoque", content);

                if (!response.IsSuccessStatusCode)
                {
                    Console.WriteLine($"[ERRO] Falha ao atualizar estoque. Status: {response.StatusCode}");
                }
                else
                {
