        ...
        static async Task Main(string[] args)
        {
            ...
            consumer.Received += async (model, ea) =>
            {
                ...
                try
                {
                    ...
                    if (!pedidoStatusJson.RootElement.TryGetProperty("UsuarioId", out var usuarioIdProp) ||
                        !pedidoStatusJson.RootElement.TryGetProperty("Status", out var statusProp))
                    {
                        Console.WriteLine("[ERRO] Mensagem não contém os campos esperados.");
                        return;
                    }
                    var usuarioId = usuarioIdProp.GetInt32().ToString();
